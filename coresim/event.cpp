...
void FlowCreationForInitializationEvent::process_event() {
    ...
    double tnext = time + nv_intarr->value()*params.time_start_compress;
...
//        std::cout << "bad slowdown " << 1e6 * flow->flow_completion_time << " " << topology->get_oracle_fct(flow) << " " << slowdown << "\n";
    ...
    auto stat = flow->tracker.finalizeResults();
    if (print_flow_result() && stat.first) {
    ...
    auto error = flow->tracker.getCounterErrors();
    if (error.length() != 0)
    {
        std::cout << "Error " << error << "\n";
    }

    std::cerr << cntF++ <<  " " << event_queue.size() <<"\n";
}
